...
public class FlockUnit : MotionBase {
    ...
    public float seekRefreshDelay = 3.0f;

    public float catchUpMinDistance; //min distance to use catchup factor

    public float wanderDelay;
    public float wanderRadius;
    public bool wanderOriginLock; //wander will not go beyond origin and within radius
    ...
    private float mSeekTimeStarted;
    ...
    /// <summary>
    /// This is filled after computing movement or separate
    /// </summary>
    public int numAvoid {
        get { return mNumAvoids; }
    }
    ...
    public Vector2 wanderOrigin {
        get { return mWanderOrigin; }
        set { mWanderOrigin = value; mWanderStartTime = 0.0f;  }
    }
    ...
    protected override void FixedUpdate() {
        if(mState == State.Waypoint) {
            ...
            Vector2 desired = mSeekPath.vectorPath[mSeekCurPath] - pos;
            ...
            if(desired.sqrMagnitude < mPathRadiusSqr) {
                ...
                if(nextPath == mSeekPath.vectorPath.Count) {
                    ...
                    mSeekTimeStarted = Time.fixedTime;
                }
            }
            else if(Time.fixedTime - mSeekTimeStarted >= seekRefreshDelay) {
                //taking too long to get to next path, refresh
                mCurUpdateDelay = 0;

                Vector3 dest = mSeekPath.vectorPath[mSeekPath.vectorPath.Count - 1];
                ...
                if(mCurUpdateDelay >= updateDelay) {
                    ...
                    Vector2 sumForce = ComputeSeparate() + M8.MathUtil.Steer(body.velocity, desired * maxSpeed, maxForce, pathFactor);
                    //body.AddForce(desired * pathSpeed, ForceMode.VelocityChange);

                    //Vector2 sumForce = ComputeSeparate();
    ...
    void OnDrawGizmosSelected() {
        ...
        if(wanderOriginLock && wanderRadius > 0.0f) {
    ...
    private void SeekPathStart(Vector3 start, Vector3 dest) {
        if(AstarPath.active != null && mSeek != null) {
    ...
    private void WanderRefresh() {
        ...
        if(wanderOriginLock) {
            ...
            Vector2 dpos = mWanderOrigin - pos;
            float dist = dpos.magnitude;
            if(dist > wanderRadius) {
                SeekPathStart(pos, mWanderOrigin);
            }
            else {
    ...
    private Vector2 Wall() {
        /*Vector3 n = mWallHit.normal;
        Vector3 v = new Vector3(n.x, -n.y, n.z);
        Vector3.OrthoNormalize(ref n, ref v);*/

        Vector2 _dir = mWallHit.normal;

        //Vector2 _dir = M8.MathUtil.Reflect(dir, mWallHit.normal);
        return M8.MathUtil.Steer(body.velocity, _dir * maxSpeed, maxForce, mNumAvoids == 0 ? wallFactor : wallFactor*avoidFactor);
    }
